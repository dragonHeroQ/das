...
from automl.HyperparameterOptimizer.USH import USH, CompModelFilter
...
if __name__ == "__main__":
	...
	TimeUsed = comp_model_filter.run(X=x_train, y=y_train, fidelity_mb=0.5)

	bandit_inst, worst_loss = comp_model_filter.get_bandits_and_worst_loss()

	ush_inst = USH(time_budget-TimeUsed, bandit_inst, budget_type="time", max_number_of_round=3,
	               per_run_timelimit=240.0, evaluation_rule=evaluation_rule,
	               time_ref=start_time, worst_loss=worst_loss)
	...
	pickle.dump(learning_curve, open("{}.lcv".format(pickle_name), 'wb'))
