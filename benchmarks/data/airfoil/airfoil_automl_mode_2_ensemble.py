...
for i in range(10):
    ...
    rgr = Regressor(q_epsilon=0.5, total_timebudget=1 * 60, per_run_timebudget=60, ensemble_size=25,
...
#                 t_res.append(0)
#             else:
#                 if os.path.exists("./my_output/model_%d_%d.pkl" % (rnd, ii)):
#                     mod = joblib.load("./my_output/model_%d_%d.pkl" % (rnd, ii))
#                     logger.info("mod.get_model_name(): {}, mod.get_params(): {}"
#                                 .format(mod.get_model_name(), mod.get_params()))
