...
archi_string = "0#H@2@ExtraTreesClassifier#H@2@SVC"
...
for i in range(1, len(string_parts)):
	...
	if block[0] == 'H':
		hbcs.append(('{}'.format(i), HorizontalBlockClassifier(algo_num, algo_class)))
	else:
		hbcs.append(('{}'.format(i), VerticalBlockClassifier(algo_num, algo_class)))

alc = ArchiLayerClassifier(len(string_parts)-1, hbcs, e_id=0)
...
dac = DeepArchiClassifier(base_layer=alc, max_layer=0, early_stopping_rounds=1, n_folds=3,
                          evaluation_rule=evaluation_rule, n_classes=n_classes, e_id='dac', random_state=rng)
