...
from das.BaseAlgorithm.Regression.StackedArchiLayerRegressor import StackedArchiLayerRegressor
...
from das.BaseAlgorithm.algorithm_space import (get_algorithm_class_by_key, get_all_classification_algorithm_keys,
                                               get_all_regression_algorithm_keys, get_regression_key_dict,
                                               get_classification_key_dict)
from das.ArchitectureSearch.LearningTool.BaseLearningTool import BaseLearningTool

logger = logging.getLogger(das.logger_name)


class CompModelLearningTool(BaseLearningTool):

	def __init__(self, n_block=2, n_classes=None, evaluation_rule=None, **kwargs):
		super(CompModelLearningTool, self).__init__(n_classes=n_classes, evaluation_rule=evaluation_rule, **kwargs)
	...
	def set_parameter_space(self, ps=None):
		...
		if ps is None:
			...
			parameter_space.merge([b1_algo, b2_algo])
		else:
			tmp_space = []
			for p in ps.keys():
				ps[p].set_name(p)
	...
	def create_learning_tool(self, max_layer=1, early_stopping_rounds=1, n_folds=3, **hyper_params):
		...
		if self.is_classification:
			...
			layer_class = StackedArchiLayerRegressor
		...
		# return deep_model
		return_learning_tool = CompModelLearningTool(n_block=self.n_block,
		                                             n_classes=self.n_classes_,
		                                             evaluation_rule=self.evaluation_rule)
